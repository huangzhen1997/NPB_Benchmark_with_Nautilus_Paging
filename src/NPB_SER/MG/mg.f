...
c      open(unit=7,file='timer.flag', status='old', iostat=fstatus)
c      if (fstatus .eq. 0) then
c         timeron = .true.
c         t_names(t_init) = 'init'
c         t_names(t_bench) = 'benchmk'
c         t_names(t_mg3P) = 'mg3P'
c         t_names(t_psinv) = 'psinv'
c         t_names(t_resid) = 'resid'
c         t_names(t_rprj3) = 'rprj3'
c         t_names(t_interp) = 'interp'
c         t_names(t_norm2) = 'norm2'
c         t_names(t_comm3) = 'comm3'
c         close(7)
c      else
      timeron = .false.
c      endif
...
c      open(unit=7,file="mg.input", status="old", iostat=fstatus)
c      if (fstatus .eq. 0) then
c         write(*,50) 
c 50      format(' Reading from input file mg.input')
c         read(7,*) lt
c         read(7,*) nx(lt), ny(lt), nz(lt)
c         read(7,*) nit
c         read(7,*) (debug_vec(i),i=0,7)
c      else
c         write(*,51) 
...
c      write (*, 1001) nx(lt),ny(lt),nz(lt), Class
      call write_1001(nx(lt), ny(lt), nz(lt), class)
c      write (*, 1002) nit
      call write_1002(nit)
c      write (*, *)
...
c            write(*, 201) rnm2
c            write(*, 202) err
c 200        format(' VERIFICATION SUCCESSFUL ')
c 201        format(' L2 Norm is ', E20.13)
c 202        format(' Error is   ', E20.13)
            call write_200(rnm2, err)
         else
            verified = .FALSE.
c            write(*, 300) 
c            write(*, 301) rnm2
c            write(*, 302) verify_value
c 300        format(' VERIFICATION FAILED')
c 301        format(' L2 Norm is             ', E20.13)
c 302        format(' The correct L2 Norm is ', E20.13)
            call write_300(rnm2, verify_value)
         endif
      else
         verified = .FALSE.
c         write (*, 400)
...
c            write(*,820) 'mg-resid', t, t*100./tmax
         else
c            write(*,810) t_names(i), t, t*100./tmax
         endif
